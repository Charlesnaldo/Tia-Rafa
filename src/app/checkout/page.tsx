...
/**
 * 1. Criamos o componente de conteúdo internamente 
 * (Pode manter no mesmo arquivo, mas o segredo está no export lá embaixo)
 */
...
function CheckoutContent() {
  ...
  if (!produto) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center font-fredoka">
        ...
        <Link href="/#catalogo" className="text-purple-600 font-bold underline">Voltar ao catálogo</Link>
      </div>
    );
  }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      else throw new Error(data.error || "Erro no pagamento");
    } catch (error: any) {
      alert(error.message);
    } finally {
      setLoading(false);
          ...
          <div className="lg:col-span-7 bg-white p-8 md:p-12 rounded-[3.5rem] shadow-xl border border-gray-50">
            <div className="space-y-6">
              ...
              <button
                ...
                disabled={loading}
                className="w-full bg-[#2D3748] hover:bg-black disabled:bg-gray-300 text-white font-black py-6 rounded-[2rem] text-xl shadow-2xl transition-all flex items-center justify-center gap-4 group"
              >
                {loading ? <Loader2 className="animate-spin" /> : <>IR PARA PAGAMENTO <ArrowRight /></>}
              </button>
            </div>
          </div>
        </div>
...
/**
 * 2. O PULO DO GATO:
 * Criamos uma versão do componente que SÓ CARREGA NO CLIENTE.
 * Isso impede que a Vercel tente rodar useSearchParams no servidor durante o build.
 */
const DynamicCheckout = dynamicImport(() => Promise.resolve(CheckoutContent), {
  ssr: false,
});
